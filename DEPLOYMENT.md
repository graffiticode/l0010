...
docker build -t gcr.io/YOUR_PROJECT_ID/l0010:latest .

# Push to Container Registry
docker push gcr.io/YOUR_PROJECT_ID/l0010:latest

# Deploy to Cloud Run
gcloud run deploy l0010 \
  --image gcr.io/YOUR_PROJECT_ID/l0010:latest \
  --platform managed \
  --region us-central1 \
  --port 50010 \
...
- `PORT`: Automatically set by Cloud Run (the app defaults to 50010)
...
```bash
gcloud run services update l0010 \
  --region us-central1 \
...
gcloud run services update-traffic l0010 \
  --region us-central1 \
  --to-latest
...
1. **Port Configuration**: Ensure the app uses `process.env.PORT` and falls back to 50010
2. **Memory Limits**: Cloud Run defaults to 256MB. Increase if needed:
   ```bash
   gcloud run services update l0010 --memory 512Mi --region us-central1
   ```
3. **Cold Starts**: Cloud Run may have cold starts. Consider minimum instances:
   ```bash
   gcloud run services update l0010 --min-instances 1 --region us-central1
   ```
...
```bash
# List revisions
gcloud run revisions list --service l0010 --region us-central1
